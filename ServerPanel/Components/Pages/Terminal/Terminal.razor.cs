...
public partial class Terminal : IAsyncDisposable
{
    ...
    [
        new("Kubernetes", "ti-brand-kubernetes", [
            new("kubectl get pods -n cs2",                                          "Ver pods CS2"),
            new("kubectl get pods -n cs2 -o wide",                                  "Pods con nodo e IP"),
            new("kubectl get all -n cs2",                                           "Todo en namespace cs2"),
            new("kubectl logs -n cs2 deployment/cs2-server --tail=100",             "Últimas 100 líneas log"),
            new("kubectl logs -n cs2 deployment/cs2-server -f",                     "Seguir logs en vivo"),
            new("kubectl rollout restart deployment/cs2-server -n cs2",             "Reiniciar deployment"),
            new("kubectl rollout status deployment/cs2-server -n cs2",              "Estado del rollout"),
            new("kubectl scale deployment cs2-server -n cs2 --replicas=0",          "Parar servidor"),
            new("kubectl scale deployment cs2-server -n cs2 --replicas=1",          "Iniciar servidor"),
            new("kubectl describe pod -n cs2",                                      "Describir pod"),
            new("kubectl get events -n cs2 --sort-by=.lastTimestamp",               "Eventos del namespace"),
            new("kubectl exec -it -n cs2 deployment/cs2-server -- bash",            "Entrar al pod"),
            new("kubectl top pod -n cs2",                                           "CPU/RAM del pod"),
            new("kubectl top node",                                                 "CPU/RAM del nodo"),
            new("kubectl get deployment cs2-server -n cs2 -o yaml",                "YAML del deployment"),
            new("kubectl apply -f /root/cs2-deployment.yaml",                      "Aplicar deployment.yaml"),
        ]),
        new("Sistema", "ti-cpu", [
        ...
        new("Ficheros", "ti-folder", [
            new("ls -lah /root/cs2-config/cfg",                      "Configs del servidor"),
            new("ls -lah /root/cs2-config/cssharp/configs",          "Configs plugins CSS"),
            new("ls -lah /root/cs2-config/cssharp/plugins",          "Plugins instalados"),
            new("du -sh /root/cs2-config",                           "Tamaño configs"),
            new("chown -R 1000:1000 /root/cs2-config",               "Arreglar permisos (steam uid)"),
            new("cat /root/cs2-config/cfg/server.cfg",               "Ver server.cfg"),
        ]),
        new("Servicio CS2", "ti-device-gamepad", [
            new("kubectl get deployment cs2-server -n cs2",           "Estado deployment"),
            new("kubectl describe deployment cs2-server -n cs2",      "Detalles deployment"),
            new("kubectl get pod -n cs2 -o jsonpath='{.items[0].metadata.name}'", "Nombre del pod"),
            new("ls -lah /root/cs2-config/cssharp/configs/admins.json", "Admins CSS"),
        ]),
